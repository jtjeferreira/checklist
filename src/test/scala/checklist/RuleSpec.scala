...
import scalaz.{\&/ => Ior, NonEmptyList}
...
class ConverterRulesSpec extends FreeSpec with Matchers {
  "parseInt" in {
    val rule = parseInt
    rule("abc")   should be(Ior.This(errors("Must be a whole number")))
    rule("123")   should be(Ior.That(123))
    rule("123.4") should be(Ior.This(errors("Must be a whole number")))
  }

  "parseDouble" in {
    val rule = parseDouble
    rule("abc")   should be(Ior.This(errors("Must be a number")))
    rule("123")   should be(Ior.That(123.0))
    rule("123.4") should be(Ior.That(123.4))
  }

  "mapValue" - {
    "string keys" in {
      val rule = mapValue[String, String]("foo")
      rule(Map.empty) should be(Ior.This(errors("foo" -> "Value not found")))
      rule(Map("foo" -> "bar")) should be(Ior.That("bar"))
      rule(Map("baz" -> "bar")) should be(Ior.This(errors("foo" -> "Value not found")))
    }

    "int keys" in {
      val rule = mapValue[Int, String](123)
      rule(Map.empty) should be(Ior.This(errors(123 -> "Value not found")))
      rule(Map(123 -> "bar")) should be(Ior.That("bar"))
      rule(Map(456 -> "bar")) should be(Ior.This(errors(123 -> "Value not found")))
    }
  }

  "trimString" in {
    val rule = trimString
    rule("") should be(Ior.That(""))
    rule("foo") should be(Ior.That("foo"))
    rule(" foo ") should be(Ior.That("foo"))
  }
}
...
class PropertyRulesSpec extends FreeSpec with Matchers {
  "seq" in {
    ...
    rule(Nil) should be(Ior.That(Nil))
    rule(List("ok", "ok")) should be(Ior.That(List("ok", "ok")))
  ...
  "opt" in {
    val rule = eql("ok").opt
    rule(None)       should be(Ior.That(None))
    rule(Some("ok")) should be(Ior.That(Some("ok")))
    rule(Some("no")) should be(Ior.Both(errors("Must be ok"), Some("no")))
  }

  "req" in {
    val rule = eql(0).req
    rule(None)    should be(Ior.This(errors("Value is required")))
    rule(Some(0)) should be(Ior.That(0))
    rule(Some(1)) should be(Ior.Both(errors("Must be 0"), 1))
  }

  "eql" in {
    val rule = eql(0)
    rule(0)  should be(Ior.That(0))
  ...
  "lengthLt"  in {
    ...
    rule("")      should be(Ior.That(""))
    rule("abcd")  should be(Ior.That("abcd"))
  ...
  "lengthLte" in {
    ...
    rule("")       should be(Ior.That(""))
    rule("abcde")  should be(Ior.That("abcde"))
  ...
  "matchesRegex" in {
    ...
    rule("dave@example.com")  should be(Ior.That("dave@example.com"))
  ...
  "containedIn" in {
    ...
    rule(1) should be(Ior.That(1))
    rule(2) should be(Ior.That(2))
    rule(3) should be(Ior.That(3))
...
class CombinatorRulesSpec extends FreeSpec with Matchers {
  ...
  "contramap" in {
    ...
    rule(+1) should be(Ior.That(2))
    rule( 0) should be(Ior.That(1))
  ...
  "flatMap" in {
    ...
    rule(-10) should be(Ior.Both(warnings("Should be >= 0") append errors("Must be > -10"), -10))
  }
  ...
  "andThen" in {
    ...
    rule(  "0") should be(Ior.Both(errors("Must be greater than 0"), 0))
    rule( "+1") should be(Ior.That(1))
    rule("1.2") should be(Ior.This(errors("Must be a whole number")))
  }

  "zip" in {
    val rule = parseInt zip parseDouble
    rule("abc") should be(Ior.This(errors("Must be a whole number", "Must be a number")))
    rule(  "1") should be(Ior.That((1, 1.0)))
    rule("1.2") should be(Ior.This(errors("Must be a whole number")))
  }

  "prefix" in {
    val rule = parseInt andThen gt(0) prefix "num"
    rule("abc") should be(Ior.This(errors("num" -> "Must be a whole number")))
    ...
    rule( "+1") should be(Ior.That(1))
    rule("1.2") should be(Ior.This(errors("num" -> "Must be a whole number")))
  }
  ...
  "composeLens" in {
    ...
    rule((-1, +1)) should be(Ior.That((-1, 1)))
  }
...
class CatsRuleSpec extends FreeSpec with Matchers {
  ...
  def getField(name: String): Rule[Map[String, String], String] =
    pure(_.get(name).map(Ior.That.apply).getOrElse(Ior.This(errors(s"Field not found: ${name}"))))
  ...
  "good data" in {
    ...
    val expected = Ior.That(Address(29, "Acacia Road"))
    actual should be(expected)
  }

  "empty data" in {
    val actual   = parseAddress(Map.empty[String, String])
    val expected = Ior.This(errors("Field not found: house", "Field not found: street"))
    actual should be(expected)
  }
...
class Rule1SyntaxSpec extends FreeSpec with Matchers {
  ...
  "field macro" in {
    ...
    rule(Coord(1, 0)) should be(Ior.That(Coord(1, 0)))
    ...
    rule(Coord(1, 1)) should be(Ior.That(Coord(1, 1)))
  }

  "fieldWith macro" in {
    val rule = Rule.pass[Coord].fieldWith(_.x)(c => gte(c.y, errors("fail")))
    rule(Coord(1, 0)) should be(Ior.That(Coord(1, 0)))
    rule(Coord(0, 0)) should be(Ior.That(Coord(0, 0)))
    rule(Coord(0, 1)) should be(Ior.Both(errors(("x" :: PNil) -> "fail"), Coord(0, 1)))
    rule(Coord(1, 1)) should be(Ior.That(Coord(1, 1)))
  }
